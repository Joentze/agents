...
const artifactSchema = z.object({
    index: z.number(),
    initialNodeMarkdown: z.string().describe("The initial markdown of the node to update"),
    instructions: z
        .string()
        .describe(
...
function updateNodeInArtifactTool({
    writer,
    messages,
}: {
    ...
    return tool({
        description: "Write the markdown block at the appropriate index based on the selected text and prompt, before using this tool, read the artifact using the readArtifact tool, if necessary also use the readNodeInArtifact tool to find an appropriate node index to update the markdown block at, if unclear ask user what to update.",
        ...
        execute: async ({ index, instructions }) => {
            ...
            writer.write({
                type: "data-artifact-agent-update-markdown-node-in-artifact-end",
                data: {
                    markdown: newArtifactMarkdown,
                    index,
                },
            });
            return `The updated markdown for node at index ${index} is: ${newArtifactMarkdown}, now summarise the changes made to the user`;
        },
    });
}

export { updateNodeInArtifactTool };